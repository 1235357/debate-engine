...
def format_consensus_as_markdown(consensus: Any) -> str:
    ...
    if metadata is not None:
        ...
        task_str = getattr(task_type, "value", str(task_type))
        lines.append(f"| Task Type | {task_str} |")
        provider_mode = getattr(metadata, "provider_mode", None)
        pm_str = getattr(provider_mode, "value", str(provider_mode))
        ...
        term_str = getattr(termination, "value", str(termination))
    ...
    if critiques:
        ...
        for sev_level in severity_order:
            ...
            for i, c in enumerate(items, 1):
                ...
                defect_str = getattr(defect, "value", str(defect))
                evidence = getattr(c, "evidence", "")
                fix = getattr(c, "suggested_fix", "")
                fix_kind = getattr(c, "fix_kind", None)
                fix_kind_str = getattr(fix_kind, "value", str(fix_kind))
    ...
    if minority:
        ...
        for m in minority:
            ...
            sev_str = getattr(sev, "value", str(sev)) if sev is not None else "?"
