...
class DebateEvalScores:
    ...
    def to_dict(self) -> dict:
        return {m.name.value: {"value": m.value, "description": m.description, "details": m.details} for m in self.metrics}
...
def compute_bdr(discovered_defects: list[dict], gold_standard: list[dict], similarity_threshold: float = 0.75) -> MetricResult:
    if not gold_standard:
        return MetricResult(name=MetricName.BDR, value=1.0, description="BDR is 1.0 when there are no gold-standard defects.", details={"discovered_count": len(discovered_defects), "gold_count": 0})
    ...
    for disc in discovered_defects:
        ...
        for gi, gold in enumerate(gold_standard):
            ...
            if score > best_score: best_score, best_idx = score, gi
        if best_score >= similarity_threshold and best_idx >= 0:
            tp_count += 1
            matched_gold.add(best_idx)
    bdr = tp_count / len(gold_standard)
    return MetricResult(name=MetricName.BDR, value=round(bdr, 4), description=f"Bug Discovery Rate: {tp_count}/{len(gold_standard)} gold defects discovered.", details={"true_positives": tp_count, "gold_count": len(gold_standard), "discovered_count": len(discovered_defects)})


def compute_far(discovered_defects: list[dict], gold_standard: list[dict], similarity_threshold: float = 0.75) -> MetricResult:
    if not discovered_defects:
        return MetricResult(name=MetricName.FAR, value=0.0, description="FAR is 0.0 when no defects were discovered.", details={"discovered_count": 0, "gold_count": len(gold_standard)})
    matched_gold: set[int] = set()
    false_alarm_count = 0
    for disc in discovered_defects:
        best_score, best_idx = 0.0, -1
    ...
    return MetricResult(name=MetricName.FAR, value=round(far, 4), description=f"False Alarm Rate: {false_alarm_count}/{len(discovered_defects)} discovered defects are false alarms.", details={"false_alarms": false_alarm_count, "discovered_count": len(discovered_defects), "gold_count": len(gold_standard)})
...
def compute_cv(consensus_conclusion: str, reference_answer: str) -> MetricResult:
    ...
    return MetricResult(name=MetricName.CV, value=round(score, 4), description=f"Consensus Validity: {score:.2%} keyword overlap between consensus and reference answer.", details={"consensus_length": len(consensus_conclusion), "reference_length": len(reference_answer)})
...
def compute_cis(revision_history: list[dict], critiques: list[dict]) -> MetricResult:
    ...
    if not revision_history:
        return MetricResult(name=MetricName.CIS, value=0.0, description="CIS is 0.0 when there is no revision history.", details={"revision_count": 0, "critique_count": len(critiques)})
    ...
    for rev in revision_history:
        ...
        if not old_claim and not new_claim: continue
        stance_delta = 1.0 - _keyword_overlap_score(old_claim, new_claim)
        if stance_delta < 0.01: continue
    ...
    if total_stance_changes == 0:
        return MetricResult(name=MetricName.CIS, value=0.0, description="CIS is 0.0 when there are no meaningful stance changes.", details={"revision_count": len(revision_history), "critique_count": len(critiques)})
    cis = 1.0 - (adopted_high_severity_changes / total_stance_changes)
    return MetricResult(name=MetricName.CIS, value=round(cis, 4), description=f"Conformity Impact Score: {cis:.4f} ({'low' if cis < 0.3 else 'moderate' if cis < 0.7 else 'high'} degradation from sycophantic stance changes).", details={"adopted_high_severity_changes": adopted_high_severity_changes, "total_stance_changes": total_stance_changes, "revision_count": len(revision_history), "critique_count": len(critiques)})


def compute_ce(cv_score: float, rounds_completed: int, total_cost_usd: float) -> MetricResult:
    if rounds_completed <= 0:
        return MetricResult(name=MetricName.CE, value=0.0, description="CE is 0.0 when no rounds were completed.", details={"cv_score": cv_score, "rounds": 0, "cost_usd": total_cost_usd})
    if total_cost_usd <= 0.0:
        ce = cv_score / rounds_completed
        return MetricResult(name=MetricName.CE, value=round(ce, 4), description=f"Convergence Efficiency: {ce:.4f} (cost was zero).", details={"cv_score": cv_score, "rounds": rounds_completed, "cost_usd": 0.0})
    ce = cv_score / (rounds_completed * total_cost_usd)
    return MetricResult(name=MetricName.CE, value=round(ce, 4), description=f"Convergence Efficiency: {ce:.4f} (CV={cv_score:.4f}, rounds={rounds_completed}, cost=${total_cost_usd:.4f}).", details={"cv_score": cv_score, "rounds": rounds_completed, "cost_usd": total_cost_usd})


def compute_rd(critiques: list[dict], adopted_count: int = 0) -> MetricResult:
    if not critiques:
        return MetricResult(name=MetricName.RD, value=0.0, description="RD is 0.0 when there are no critiques.", details={"total_critiques": 0})
    ...
    return MetricResult(name=MetricName.RD, value=round(rd, 4), description=desc, details={"concrete_fix_count": concrete_count, "total_critiques": total_count, "concrete_fix_ratio": round(concrete_ratio, 4), "adopted_count": adopted_count})
...
def compute_hd(debate_faithfulness: float, baseline_faithfulness: float) -> MetricResult:
    ...
    return MetricResult(name=MetricName.HD, value=round(hd, 4), description=f"Hallucination Delta: {hd:+.4f} (debate={debate_faithfulness:.4f}, baseline={baseline_faithfulness:.4f}, {qualifier} hallucinations than baseline).", details={"debate_faithfulness": debate_faithfulness, "baseline_faithfulness": baseline_faithfulness})